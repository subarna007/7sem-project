...
<body>
    ...
    <div class="block bg-light" id="foods">
        ...
        <div class="blocks">
          <?php
          $conn=new DBConnect();
             $result2=$conn->own_query("Select f.id,f.food_name,f.cost,f.food_photo,f.category from food f inner join food_order fo group by f.food_name order by sum(qty) limit 4");
             $i=0;
             if(count($result2)>0){
              ...
              $result2=$conn->own_query("Select * from food limit 4");
              $i=0;
              if(count($result2)>0){
               foreach($result2 as $row2){
                 food_card($row2["food_name"],"img/foods/".$row2["food_photo"],$row2["cost"]);
