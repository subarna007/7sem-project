      ...
      foreach($result as $row){
       ...
       $package=array();
        ...
        if($row["payment_date"]=="0"&&date("Y/m/d")<=str_replace("-","/",$row["check_out_day"])){
          if(count($package)==0)echo "<td><button class='checkout12' onclick='checkout(\"".$row['user_id']."\",\"".$row["check_in_date"]."\",\"".$row["check_out_day"]."\")'>Pay Bills</button></td>";
        else echo "<td><button class='checkout12' onclick='checkout(\"".$row['user_id']."\",\"".$row["check_in_date"]."\",\"".$row["check_out_date"]."\",\"".$package[0]."\",\"".$package[1]."\")'>Pay Bills</button></td>";
        echo "</tr>";
        }
        if($row["ratings"]=="-1"){
            echo "<a style='
