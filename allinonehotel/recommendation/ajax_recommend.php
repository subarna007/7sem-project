	...
	if(count($product)>=1){
		...
		foreach($product as $p){
			if($p["ratings"]>=0||$p["user_id"]==$_COOKIE["customer_id"])
			{
				...
				$username=$users[0];
				$matrix[$username['username']][$p['room_name']]=$p['ratings'];  
			}
	//    print_r($matrix);
	   }
	   ...
	   foreach($recommendation as $product=>$rating)
		   {
			//    $arr2=array("roomname"=>$product,"rating"=>$rating);
			   ...
			   foreach($result3 as $row3){
				...
				//    print_r($result4);
				   $arr2=array("roomname"=>$row3["room_name"],"room_pic"=>$row3["room_pic"],"price"=>$row3["price"],"ratings"=>$result4[0]["avg(ratings)"]);
