...
$conn=new DBConnect();
 if(isset($_GET["id"])){
    if(!empty($_GET["id"])||$_GET["id"]!=""){
        $result1=$conn->own_query("Select b.room_name,b.price,b.payment_date,u.id, u.full_name, u.username, u.address, u.contact from user_details u inner join bookings b where b.user_id=u.id and check_in_date='".$_GET["cin"]."' and check_out_day='".$_GET["cout"]."' and b.user_id=".$_GET["id"]);
        // echo "Select b.room_name,b.price,b.payment_date,u.id, u.full_name, u.username, u.address, u.contact from user_details u inner join bookings b where b.user_id=u.id and check_in_date='".$_GET["cin"]."' and check_out_day='".$_GET["cout"]."' and b.user_id=".$_GET["id"];
        // print_r($result1);
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="text-center text-150">
                            ...
                            <h3><center>Gwarkho,Kathmandu</center></h3>
                        </div>
                    </div>
                </div>
                <!-- .row -->
                    ...
                    <div class="text-95 col-sm-6 align-self-start d-sm-flex justify-content-end">
                        ...
                        <div class="text-grey-m2">
                            ...
                            if($row1["payment_date"]==0){
                                $payment_status='<span class="badge badge-warning badge-pill px-25">Unpaid</span>';
                            }
                            else{
                                $payment_status='<span class="badge badge-warning badge-pill px-25">Paid</span>';
                            }
                            echo $payment_status;
                            ?></div>
                        </div>
                ...
                <div class="mt-4">
                               ...
                               if(isset($_GET["package"])){
                                if($_GET["package"]=="no"){
                                        ...
                                        <div class="row mb-2 mb-sm-0 py-25 bgc-default-l4">
                                            ...
                                            <div class="d-none d-sm-block col-2 text-95">NRs-'.$row1["price"].'</div>
                                            <div class="col-2 text-secondary-d2">NRs-';
                                            echo $row1["price"]; $price=(float)$row1["price"]; $total+=$price;
                                            echo '</div>
                                        </div>                                    
                                        ';

                                        $result2=$conn->own_query("Select * from food_order where user_id=".$_GET["id"]." and date2>='".str_replace("-","/",$_GET['cin'])."' and date2<='".str_replace("-","/",$_GET["cout"])."'");
                                            if(count($result2)>0){
                                        ...
                                        <div class="row mb-2 mb-sm-0 py-25 bgc-default-l4">
                                            <div class="d-none d-sm-block col-1">'.$i.'</div>
                                            <div class="col-9 col-sm-5">'.ucfirst($_GET["package"]).'</div>
                                            <div class="d-none d-sm-block col-2">1</div>
                    ...
                    <div>
                        ...
                        <form action="https://uat.esewa.com.np/epay/main" method="POST">
                                <input  name="tAmt" id="tAmt" type="hidden" value="<?=$t_total?>">
                                <input id="amt" name="amt" type="hidden" value="<?=$total?>">
                                <input name="txAmt" id="txAmt" type="hidden" value="<?=$tax?>">
                                <input value="0" name="psc" type="hidden">
                                <input value="0" name="pdc" type="hidden">
                                <input value="EPAYTEST" name="scd" type="hidden">
                                <input value="Harke121232" name="pid" type="hidden">
                                <?php echo ' <input value="http://localhost/harke/allinonehotel/payment/success.php?id='.$_GET["id"].'&cin='.$_GET["cin"].'&cout='.$_GET["cout"].'&package='.$_GET["package"].'&pkg_price='.$_GET["pkg_price"].'" type="hidden" name="su">
